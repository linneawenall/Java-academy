...
public class Broker implements Runnable {
	...
	public Broker() {
		hitStock = new HitStock();
	}

	@Override
	public void run() {
		System.out.println("Entering run in Broker for " + Thread.currentThread().getName());
		if (Math.random() > 0.5) {
			try {
	...
	public int getRandomAmount() {
		System.out.println("In getRandomAmount in Broker for " + Thread.currentThread().getName());
		return (int) (Math.random() * hitStock.getStartAmount() * 2);
	}
}
